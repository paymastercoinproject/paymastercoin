...
Building with Visual Studio is an alternative to the Linux based [cross-compiler build](https://github.com/adminpaymastercoin/paymastercoin/blob/master/doc/build-windows.md).
...
msbuild /m build_msvc\paymastercoin.sln /p:Platform=x64 /p:Configuration=Release /t:build
```

Dependencies
---------------------
A number of [open source libraries](https://github.com/adminpaymastercoin/paymastercoin/blob/master/doc/dependencies.md) are required in order to be able to build Bitcoin Core.
...
The [external dependencies](https://github.com/adminpaymastercoin/paymastercoin/blob/master/doc/dependencies.md) required for building are:
...
To build Bitcoin Core without Qt unload or disable the `paymastercoin-qt`, `libpaymastercoin_qt` and `test_paymastercoin-qt` projects.
...
```
msbuild /m paymastercoin.sln /p:Platform=x64 /p:Configuration=Release /p:PlatformToolset=v141 /t:build
```

- To build from the command line with the Visual Studio 2019 toolchain use:

```
msbuild /m paymastercoin.sln /p:Platform=x64 /p:Configuration=Release /t:build
```

- Alternatively open the `build_msvc\paymastercoin.sln` file in Visual Studio.
...
```
    #- 7z a paymastercoin-%APPVEYOR_BUILD_VERSION%.zip %APPVEYOR_BUILD_FOLDER%\build_msvc\%platform%\%configuration%\*.exe
    #- path: paymastercoin-%APPVEYOR_BUILD_VERSION%.zip
```
