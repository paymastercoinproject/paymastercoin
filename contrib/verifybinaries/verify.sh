...
###   paymastercoincore.org and paymastercoin.org and compares them.
...
WORKINGDIR="/tmp/paymastercoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://paymastercoincore.org"
HOST2="https://paymastercoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="paymastercoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'paymastercoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "paymastercoin.org failed to provide signature file, but paymastercoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "paymastercoin.org and paymastercoincore.org signature files were not equal?"
