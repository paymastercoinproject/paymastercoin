...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!m_pool.CalculateMemPoolAncestors(*entry, setAncestors, m_limit_ancestors, m_limit_ancestor_size, m_limit_descendants, m_limit_descendant_size, errString)) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/paymastercoin-dev/2018-November/016518.html
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if(nHeight == 1) // first Block
    {
       nSubsidy = 9000000000  * COIN; // 90 000 000 Premine
    }   
    else if(nHeight<=1000)
    {
        nSubsidy = 80000 * COIN; // 79 920 000
    }
    else if(nHeight<=10000)
    {
        nSubsidy = 40000 * COIN; // 360 000 000
    }
    else if(nHeight<=20000)
    {
        nSubsidy = 20000 * COIN; // 200 000 000
    }  
    else if(nHeight<=50000)
    {
        nSubsidy = 10000 * COIN; // 300 000 000
    }  
    else if(nHeight<=90000)
    {
        nSubsidy = 5000 * COIN; // 200 000 000
    }
    else if(nHeight<=160000)
    {
        nSubsidy = 2500 * COIN; // 175 000 000
    } 
    else if(nHeight<=320000)
    {
        nSubsidy = 1250 * COIN; // 200 000 000
    } 
    else if(nHeight<=106300106)
    {
        nSubsidy = 750 * COIN; // 79 485 079 500
    }   
    else
    {
        nSubsidy = 0 * COIN;
