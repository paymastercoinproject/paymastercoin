...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. paymastercoin_de.qm (shortcut "de" needs to be defined in paymastercoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. paymastercoin_de_DE.qm (shortcut "de_DE" needs to be defined in paymastercoin.qrc)
...
static const char* qt_argv = "paymastercoin-qt";
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // paymastercoin: URIs or payment requests:
        if (paymentServer) {
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(paymastercoin);
    Q_INIT_RESOURCE(paymastercoin_locale);
    ...
    /// 6. Determine availability of data directory and parse paymastercoin.conf
    ...
    // paymastercoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
