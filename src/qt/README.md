...
ClientModel is used by the main application `paymastercoingui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a paymastercoin:... link from a browser).
...
* `paymastercoinstrings.cpp`: automatically generated
* `paymastercoinunits.(h/cpp)`: PMC / mPMC / etc handling
* `callback.h`
...
* don't change `local/paymastercoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "paymastercoin-qt" under "Run", which is where you can also add command line arguments)
