...
const char * const BITCOIN_CONF_FILENAME = "paymastercoin.conf";
 ...
 * https://github.com/adminpaymastercoin/paymastercoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //paymastercoin-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "paymastercoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PaymasterCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PaymasterCoin
    // Mac: ~/Library/Application Support/PaymasterCoin
    // Unix: ~/.paymastercoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PaymasterCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PaymasterCoin";
#else
    // Unix
    return pathRet / ".paymastercoin";
#endif
#endif
}
