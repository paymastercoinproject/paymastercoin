...
Bitcoin Core version 0.19.0 is now available from:
  ...
  <https://paymastercoincore.org/bin/paymastercoin-core-0.19.0/>
=======
...
Bitcoin Core version 0.20.0 is now available from:
  ...
  <https://paymastercoincore.org/bin/paymastercoin-core-0.20.0/>
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/adminpaymastercoin/paymastercoin/issues/16599),
  [PR #16702](https://github.com/adminpaymastercoin/paymastercoin/pull/16702), and the
  `paymastercoind help` for more information.  This option is experimental and
...
- A "known bug" [announced](https://paymastercoincore.org/en/releases/0.18.0/#wallet-gui)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  program such as [HWI](https://github.com/paymastercoin-core/HWI) for
...
- Bitcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.paymastercoincore.org.  (#17596)
...
- The `paymastercoin-cli` utility used with the `-getinfo` parameter now
...
- The `paymastercoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `paymastercoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_paymastercoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_paymastercoin` binary (hebasto)
...
- #18702 Fix ASLR for paymastercoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate paymastercoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest paymastercoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_paymastercoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (paymastercoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone paymastercoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #17119 Fix broken paymastercoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from paymastercoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.paymastercoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to paymastercoin-wallet (hel-o)
...
- #16115 On paymastercoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from paymastercoin-cli -getinfo (malevolent)
...
- #15639 paymastercoin-wallet tool: Drop libpaymastercoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in paymastercoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "paymastercoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of paymastercoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from paymastercoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_paymastercoin to auto generated project files (sipsorcery)
...
- #15352 Reduce noise level in test_paymastercoin output (practicalswift)
...
- #15896 feature_filelock, interface_paymastercoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Bitcoin Core (luke-jr)
...
- #16277 Suppress output in test_paymastercoin for expected errors (gertjaap)
...
- #16489 log: harmonize paymastercoind logging (jonatack)
...
- #15513 add "sections" info to example paymastercoin.conf (fanquake)
...
- #16080 build/doc: update paymastercoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in paymastercoin.conf (fanquake)
...
- #16621 add default paymastercoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the paymastercoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, paymastercoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/paymastercoind.cpp (ch4ot1c)
...
[Transifex](https://www.transifex.com/paymastercoin/paymastercoin/).
